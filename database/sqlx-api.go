...
func selectDemo() {
	...
	err := sqlDb.Select(&users, sqlStr, 0)
...
func execInsert() {
	...
	ret, err := sqlDb.Exec(sqlStr, "Jerry", 18)
...
func execUpdate() {
	...
	ret, err := sqlDb.Exec(sqlStr, 22, 1)
	if err != nil {
...
func queryDemo() {
	rows, err := sqlDb.Query("select name, age from user")
	if err != nil {
...
func queryxDemo() {
	rows, err := sqlDb.Queryx("select id, name, age from user")
	if err != nil {
...
func queryRowAndQueryRowx() {
	row := sqlDb.QueryRow("select id, name, age FROM user where id = ?", 3) // QueryRow返回错误，错误通过Scan返回
	var id int
	var name string
	var age int
	...
	var u User
	err = sqlDb.QueryRowx("select id, name, age FROM user where id = ?", 2).StructScan(&u)
	if err != nil {
...
func queryByIDs(ids []int) (users []User, err error) {
	...
	err = sqlDb.Select(&users, query, args...)
	return
}
...
func BatchInsertUsers3(users []*User) error {
	_, err := sqlDb.NamedExec("INSERT INTO user (name, age) VALUES (:name, :age)", users)
	return err
}
